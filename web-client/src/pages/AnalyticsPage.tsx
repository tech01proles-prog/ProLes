...
export function AnalyticsPage() {
  ...
  return (
    <div className="analytics-page p-6 space-y-6">
      ...
      {selectedEmployee && employeeDetail && createPortal(
        <div className="analytics-detail-overlay" onClick={() => setSelectedEmployeeId(null)}>
          <div className="analytics-detail-panel employee-panel" onClick={e => e.stopPropagation()}>
            <div className="analytics-detail-header">
              <div className="min-w-0">
                <span className="text-xs font-bold uppercase tracking-wider text-emerald-600">Сотрудник</span>
                <h2 className="truncate">{selectedEmployee.name}</h2>
                <p>{selectedEmployee.position} · {selectedEmployee.avgHoursPerDay.toFixed(1)} ч/день · стабильность {Math.round(selectedEmployee.stabilityScore)}%</p>
              </div>
              <button onClick={() => setSelectedEmployeeId(null)} className="analytics-close">×</button>
            </div>
            <div className="analytics-detail-grid">
              <div className="detail-tone detail-green"><span>Рабочее время</span><strong>{formatDuration(selectedEmployee.hours)}</strong></div>
              <div className="detail-tone detail-blue"><span>Рабочих дней</span><strong>{selectedEmployee.workDays}</strong></div>
              <div className="detail-tone detail-amber"><span>Расходы</span><strong>{formatMoneyExact(selectedEmployee.expenseRub)}</strong></div>
              <div className="detail-tone detail-red"><span>Без чека</span><strong>{formatMoneyExact(selectedEmployee.noReceiptRub)}</strong></div>
            </div>
            <div className="analytics-detail-section"><h3>По проектам</h3><div className="space-y-2">{employeeDetail.byProject.map(p => <button key={p.projectId} onClick={() => { setSelectedEmployeeId(null); setSelectedProjectId(p.projectId); }} className="analytics-detail-row"><span className="detail-main-label"><i className="detail-dot detail-dot-green" />{p.name}</span><span className="detail-value detail-value-blue">{formatDuration(p.hours)} · {formatMoneyExact(p.expense)}</span></button>)}</div></div>
            <div className="analytics-detail-section"><h3>Время</h3><div className="analytics-detail-scroll">{employeeDetail.userEntries.map(e => <div key={`${e.userId}-${e.projectId}-${e.date}-${e.hours}`} className="analytics-detail-row"><span><span className="detail-date">{new Date(e.date).toLocaleDateString('ru-RU')}</span> · {e.projectName || 'Без проекта'}</span><strong className="detail-value-green">{formatDuration(Number(e.hours || 0))}</strong></div>)}</div></div>
            <div className="analytics-detail-section"><h3>Расходы</h3><div className="analytics-detail-scroll">{employeeDetail.userExpenses.map(e => <div key={e.id} className="analytics-detail-row"><span><span className="detail-date detail-date-expense">{new Date(e.date).toLocaleDateString('ru-RU')}</span> · {e.name?.trim() || expenseLabel(e)}</span><strong className="detail-value-amber">{formatMoneyExact(rub(Number(e.amount || 0), e.currency))}</strong></div>)}</div></div>
          </div>
        </div>,
        document.body
      )}
      {selectedProject && projectDetail && createPortal(
        <div className="analytics-detail-overlay" onClick={() => setSelectedProjectId(null)}>
          <div className="analytics-detail-panel" onClick={e => e.stopPropagation()}>
            <div className="analytics-detail-header">
              <div className="min-w-0">
                <span className="text-xs font-bold uppercase tracking-wider text-emerald-600">Проект</span>
                <h2 className="truncate">{selectedProject.name}</h2>
                <p>{formatDuration(selectedProject.hours)} · {selectedProject.employeeCount} сотрудников · расходы {formatMoneyExact(selectedProject.expenseRub)}</p>
              </div>
              <button onClick={() => setSelectedProjectId(null)} className="analytics-close">×</button>
            </div>
            <div className="analytics-detail-grid">
              <div className="detail-tone detail-amber"><span>Расходы</span><strong>{formatMoneyExact(selectedProject.expenseRub)}</strong></div>
            ...
            <div className="analytics-detail-section"><h3>Кто работал</h3><div className="space-y-2">{projectDetail.byEmployee.map(p => <button key={p.userId} onClick={() => { setSelectedProjectId(null); setSelectedEmployeeId(p.userId); }} className="analytics-detail-row"><span className="detail-main-label"><i className="detail-dot detail-dot-blue" />{shortPersonName(p.name)}</span><span className="detail-value detail-value-blue">{formatDuration(p.hours)} · {formatMoneyExact(p.expense)}</span></button>)}</div></div>
            <div className="analytics-detail-section"><h3>Рабочее время</h3><div className="analytics-detail-scroll">{projectDetail.projectEntries.map(e => <div key={`${e.userId}-${e.date}-${e.hours}`} className="analytics-detail-row"><span><span className="detail-date">{new Date(e.date).toLocaleDateString('ru-RU')}</span> · {shortPersonName(users.find(u => u.id === e.userId)?.name || e.userName || '—')}</span><strong className="detail-value-green">{formatDuration(Number(e.hours || 0))}</strong></div>)}</div></div>
            <div className="analytics-detail-section"><h3>Все расходы</h3><div className="analytics-detail-scroll">{projectDetail.projectExpenses.map(e => <div key={e.id} className="analytics-detail-row"><span><span className="detail-date detail-date-expense">{new Date(e.date).toLocaleDateString('ru-RU')}</span> · {shortPersonName(users.find(u => u.id === e.userId)?.name || '—')} · {e.name?.trim() || expenseLabel(e)}</span><strong className="detail-value-amber">{formatMoneyExact(rub(Number(e.amount || 0), e.currency))}</strong></div>)}</div></div>
          </div>
        </div>,
        document.body
      )}
