      ...
      {selectedProject && projectDetail && createPortal(
        <div className="analytics-detail-overlay" onClick={() => setSelectedProjectId(null)}>
          <div className="analytics-detail-panel" onClick={e => e.stopPropagation()}>
            ...
            <div className="analytics-detail-section"><h3>Рабочее время</h3><div className="analytics-detail-scroll">{projectDetail.projectEntries.map(e => <div key={`${e.userId}-${e.date}-${e.hours}`} className="analytics-detail-row"><span><span className="detail-date">{new Date(e.date).toLocaleDateString('ru-RU')}</span> · {shortPersonName(e.userName || '—')}</span><strong className="detail-value-green">{formatDuration(Number(e.hours || 0))}</strong></div>)}</div></div>
